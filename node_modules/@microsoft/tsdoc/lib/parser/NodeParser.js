...
var NodeParser = /** @class */ (function () {
        ...
        while (!done) {
            ...
            switch (tokenReader.peekTokenKind()) {
                ...
                case Token_1.TokenKind.LeftCurlyBracket: {
                    ...
                    if (docNode instanceof nodes_1.DocInheritDocTag) {
                        ...
                        if (docComment.inheritDocTag === undefined) {
                            ...
                            this._pushNode(this._backtrackAndCreateErrorRange(tokenReader, marker, tagEndMarker, 'A doc comment cannot have more than one @inheritDoc tag'));
                        }
                    }
                    else {
                ...
                case Token_1.TokenKind.RightCurlyBracket:
                    this._pushAccumulatedPlainText(tokenReader);
                    this._pushNode(this._createError(tokenReader, 'The "}" character should be escaped using a backslash to avoid confusion with a TSDoc inline tag'));
                    break;
                ...
                case Token_1.TokenKind.GreaterThan:
                    this._pushAccumulatedPlainText(tokenReader);
                    this._pushNode(this._createError(tokenReader, 'The ">" character should be escaped using a backslash to avoid confusion with an HTML tag'));
                    break;
    ...
    NodeParser.prototype._parseParamBlock = function (tokenReader, docBlockTag) {
        ...
        if (parameterName.length === 0) {
            ...
            this._parserContext.log.addMessageForTokenSequence('The @param block should be followed by a parameter name', docBlockTag.getTokenSequence(), docBlockTag);
        ...
        if (tokenReader.peekTokenKind() !== Token_1.TokenKind.Hyphen) {
            tokenReader.backtrackToMarker(startMarker);
            this._parserContext.log.addMessageForTokenSequence('The @param block should be followed by a parameter name and then a hyphen', docBlockTag.getTokenSequence(), docBlockTag);
    ...
    NodeParser.prototype._parseInheritDocTag = function (docInlineTagParsedParameters, embeddedTokenReader) {
        ...
        if (embeddedTokenReader.peekTokenKind() !== Token_1.TokenKind.EndOfInput) {
            ...
            if (embeddedTokenReader.peekTokenKind() !== Token_1.TokenKind.EndOfInput) {
                ...
                this._parserContext.log.addMessageForTokenSequence('Unexpected character after declaration reference', embeddedTokenReader.extractAccumulatedSequence(), errorTag);
                return errorTag;
            }
        }
    ...
    NodeParser.prototype._parseLinkTag = function (docInlineTagParsedParameters, embeddedTokenReader) {
        ...
        if (!docInlineTagParsedParameters.tagContentExcerpt) {
            this._parserContext.log.addMessageForTokenSequence('The @link tag content is missing', parameters.tagNameExcerpt, errorTag);
            return errorTag;
        }
        ...
        if (embeddedTokenReader.peekTokenKind() === Token_1.TokenKind.Pipe) {
            ...
            while (!done) {
                switch (embeddedTokenReader.peekTokenKind()) {
                    ...
                    case Token_1.TokenKind.LeftCurlyBracket:
                        ...
                        this._parserContext.log.addMessageForTokenSequence("The \"" + badCharacter + "\" character may not be used in the link text without escaping it", embeddedTokenReader.extractAccumulatedSequence(), errorTag);
                        return errorTag;
                    case Token_1.TokenKind.Spacing:
                    case Token_1.TokenKind.Newline:
                        embeddedTokenReader.readToken();
            ...
            embeddedTokenReader.readToken();
            this._parserContext.log.addMessageForTokenSequence('Unexpected character after link destination', embeddedTokenReader.extractAccumulatedSequence(), errorTag);
            return errorTag;
        }
    ...
    NodeParser.prototype._parseLinkTagUrlDestination = function (embeddedTokenReader, parameters, tokenSequenceForErrorContext, nodeForErrorContext) {
        ...
        if (invalidUrlExplanation) {
            this._parserContext.log.addMessageForTokenSequence(invalidUrlExplanation, urlDestinationExcerpt, nodeForErrorContext);
            return false;
        }
        ...
        if (hasHash) {
            ...
            if (tokenReader.peekTokenKind() !== Token_1.TokenKind.Period) {
                ...
                if (!tokenReader.isAccumulatedSequenceEmpty()) {
                    ...
                    if (explanation) {
                        this._parserContext.log.addMessageForTokenSequence(explanation, packageNameExcerpt, nodeForErrorContext);
                        return undefined;
                    }
                }
            }
            ...
            if (!tokenReader.isAccumulatedSequenceEmpty()) {
                ...
                if (explanation) {
                    this._parserContext.log.addMessageForTokenSequence(explanation, importPathExcerpt, nodeForErrorContext);
                    return undefined;
                }
            }
            ...
            if (packageNameExcerpt === undefined && importPathExcerpt === undefined) {
                this._parserContext.log.addMessageForTokenSequence('The hash character must be preceded by a package name or import path', importHashExcerpt, nodeForErrorContext);
                return undefined;
            }
        }
    ...
    NodeParser.prototype._parseMemberReference = function (tokenReader, expectingDot, tokenSequenceForErrorContext, nodeForErrorContext) {
        ...
        if (tokenReader.peekTokenKind() === Token_1.TokenKind.Colon) {
            ...
            if (!parameters.leftParenthesisExcerpt) {
                ...
                this._parserContext.log.addMessageForTokenSequence('Syntax error in declaration reference: the member selector must be enclosed in parentheses', parameters.colonExcerpt, nodeForErrorContext);
                return undefined;
            }
        ...
        if (parameters.leftParenthesisExcerpt) {
            if (tokenReader.peekTokenKind() !== Token_1.TokenKind.RightParenthesis) {
                this._parserContext.log.addMessageForTokenSequence('Expecting a matching right parenthesis', parameters.leftParenthesisExcerpt, nodeForErrorContext);
                return undefined;
            }
            tokenReader.readToken();
    ...
    NodeParser.prototype._parseMemberSymbol = function (tokenReader, nodeForErrorContext) {
        ...
        if (!declarationReference) {
            this._parserContext.log.addMessageForTokenSequence('Missing declaration reference in symbol reference', leftBracketExcerpt, nodeForErrorContext);
            return undefined;
        }
        ...
        if (tokenReader.peekTokenKind() !== Token_1.TokenKind.RightSquareBracket) {
            this._parserContext.log.addMessageForTokenSequence('Missing closing square bracket for symbol reference', leftBracketExcerpt, nodeForErrorContext);
            return undefined;
        }
        tokenReader.readToken();
    ...
    NodeParser.prototype._parseMemberIdentifier = function (tokenReader, tokenSequenceForErrorContext, nodeForErrorContext) {
        ...
        if (tokenReader.peekTokenKind() === Token_1.TokenKind.DoubleQuote) {
            ...
            while (tokenReader.peekTokenKind() !== Token_1.TokenKind.DoubleQuote) {
                if (tokenReader.peekTokenKind() === Token_1.TokenKind.EndOfInput) {
                    this._parserContext.log.addMessageForTokenSequence('Unexpected end of input inside quoted member identifier', leftQuoteExcerpt, nodeForErrorContext);
                    return undefined;
                }
                tokenReader.readToken();
            }
            if (tokenReader.isAccumulatedSequenceEmpty()) {
                this._parserContext.log.addMessageForTokenSequence('The quoted identifier cannot be empty', leftQuoteExcerpt, nodeForErrorContext);
                return undefined;
            }
            ...
            if (explanation) {
                this._parserContext.log.addMessageForTokenSequence(explanation, identifierExcerpt, nodeForErrorContext);
                return undefined;
            }
            return new nodes_1.DocMemberIdentifier({
                parsed: true,
    ...
    NodeParser.prototype._parseMemberSelector = function (tokenReader, tokenSequenceForErrorContext, nodeForErrorContext) {
        ...
        if (docMemberSelector.errorMessage) {
            this._parserContext.log.addMessageForTokenSequence(docMemberSelector.errorMessage, selectorExcerpt, nodeForErrorContext);
            return undefined;
        }
