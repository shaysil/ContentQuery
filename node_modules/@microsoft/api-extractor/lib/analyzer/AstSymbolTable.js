...
class AstSymbolTable {
    constructor(program, typeChecker, packageJsonLookup, logger) {
        /**
        ...
        this._packageMetadataManager = new PackageMetadataManager_1.PackageMetadataManager(packageJsonLookup, logger);
     ...
     * object.
     */
    fetchEntryPointModule(sourceFile) {
            ...
            case ts.SyntaxKind.ComputedPropertyName: // used for EcmaScript "symbols", e.g. "[toPrimitive]".
                {
                    ...
                    const referencedAstSymbol = this.fetchReferencedAstSymbol(symbol, symbolNode.getSourceFile());
    ...
    _fetchAstSymbol(followedSymbol, addIfMissing, astImportOptions, localName) {
        ...
        if (!astSymbol) {
            if (!followedSymbol.declarations || followedSymbol.declarations.length < 1) {
                throw new node_core_library_1.InternalError('Followed a symbol with no declarations');
            }
            const astImport = astImportOptions ? new AstImport_1.AstImport(astImportOptions) : undefined;
            if (astImport) {
                if (!astSymbol) {
            ...
            if (!astSymbol) {
                ...
                const followedSymbolSourceFile = followedSymbol.declarations[0].getSourceFile();
                ...
                if (!nominalAnalysis) {
                    for (const declaration of followedSymbol.declarations || []) {
                        if (!AstDeclaration_1.AstDeclaration.isSupportedSyntaxKind(declaration.kind)) {
                            throw new node_core_library_1.InternalError(`The "${followedSymbol.name}" symbol uses the construct`
        ...
        if (astImportOptions && !astSymbol.imported) {
            ...
            throw new node_core_library_1.InternalError('The symbol ' + astSymbol.localName + ' is being imported'
