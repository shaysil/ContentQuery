...
class ExportAnalyzer {
    constructor(program, typeChecker, astSymbolTable) {
        this._astModulesBySourceFile = new Map();
     ...
     */
    fetchAstModuleBySourceFile(sourceFile, moduleReference) {
        ...
        let astModule = this._astModulesBySourceFile.get(sourceFile);
        if (!astModule) {
            const moduleSymbol = this._getSymbolForSourceFile(sourceFile, moduleReference);
            ...
            if (astModule.isExternal) {
                ...
                for (const exportedSymbol of this._typeChecker.getExportsOfModule(moduleSymbol)) {
                    ...
                    if (TypeScriptHelpers_1.TypeScriptHelpers.hasAnyDeclarations(followedSymbol)) {
                        const astSymbol = this._astSymbolTable.fetchAstSymbol(followedSymbol, true, astImportOptions);
                        if (!astSymbol) {
     ...
     */
    _getSymbolForSourceFile(sourceFile, moduleReference) {
        const moduleSymbol = TypeScriptInternals_1.TypeScriptInternals.tryGetSymbolForDeclaration(sourceFile);
    ...
    fetchAstModuleExportInfo(astModule) {
        if (astModule.astModuleExportInfo === undefined) {
            ...
            this._collectAllExportsRecursive(astModuleExportInfo, astModule, new Set());
                        ...
                        default:
                            ...
                            if (exportName !== ts.InternalSymbolName.Default || visitedAstModules.size === 1) {
                                if (!astModuleExportInfo.exportedLocalSymbols.has(exportSymbol.name)) {
                                    const astSymbol = this._getExportOfAstModule(exportSymbol.name, astModule);
     ...
     */
    fetchReferencedAstSymbol(symbol, sourceFile) {
        const astModule = this._astModulesBySourceFile.get(sourceFile);
        if (astModule === undefined) {
            throw new node_core_library_1.InternalError('fetchReferencedAstSymbol() called for a source file that was not analyzed');
        }
        return this._fetchAstSymbolFromModule(astModule, symbol);
    }
    _fetchAstSymbolFromModule(astModule, symbol) {
        let current = symbol;
        while (true) { // tslint:disable-line:no-constant-condition
            // Is this symbol an import/export that we need to follow to find the real declaration?
            for (const declaration of current.declarations || []) {
                let matchedAstSymbol;
        ...
        const astSymbol = this._astSymbolTable.fetchAstSymbol(current, true, undefined);
        return astSymbol;
    }
    _tryMatchExportDeclaration(declaration, declarationSymbol) {
        ...
        if (exportDeclaration) {
            ...
            if (exportDeclaration.moduleSpecifier) {
                const specifierAstModule = this._fetchSpecifierAstModule(exportDeclaration, declarationSymbol);
                const astSymbol = this._getExportOfAstModule(exportName, specifierAstModule);
                return astSymbol;
            }
        }
        return undefined;
    ...
    _tryMatchImportDeclaration(declaration, declarationSymbol) {
        ...
        if (importDeclaration) {
            ...
            if (declaration.kind === ts.SyntaxKind.NamespaceImport) {
                ...
                const followedSymbol = TypeScriptHelpers_1.TypeScriptHelpers.followAliases(declarationSymbol, this._typeChecker);
                const astImportOptions = {
                    exportName: '*',
                    modulePath: specifierAstModule.externalModulePath
                };
                const astSymbol = this._astSymbolTable.fetchAstSymbol(followedSymbol, true, astImportOptions, declarationSymbol.name);
                return astSymbol;
            }
            if (declaration.kind === ts.SyntaxKind.ImportSpecifier) {
                ...
                //   StringLiteral:  pre=['./A']
                //   SemicolonToken:  pre=[;]
                const astSymbol = this._getExportOfAstModule(ts.InternalSymbolName.Default, specifierAstModule);
                return astSymbol;
            }
            else {
    ...
    _getExportOfAstModule(exportName, astModule) {
        ...
        const astSymbol = this._tryGetExportOfAstModule(exportName, astModule, visitedAstModules);
    ...
    _tryGetExportOfAstModule(exportName, astModule, visitedAstModules) {
        ...
        visitedAstModules.add(astModule);
        let astSymbol = astModule.cachedExportedSymbols.get(exportName);
        if (astSymbol !== undefined) {
            return astSymbol;
        }
     ...
     */
    _fetchSpecifierAstModule(importOrExportDeclaration, exportStarSymbol) {
        ...
        const specifierAstModule = this.fetchAstModuleBySourceFile(moduleSourceFile, {
